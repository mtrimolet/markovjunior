...
template<typename _Res, typename... _ArgTypes, bool _Noex>
  ...
  : _Mofunc_base
  {
    template<typename _Tp>
      using __callable
        = std::conditional_t<_Noex,
                          std::is_nothrow_invocable_r<_Res, _Tp, _ArgTypes...>,
                          std::is_invocable_r<_Res, _Tp, _ArgTypes...>>;

    template<typename _VT>
     static constexpr bool __is_callable_from = [] {
        using _DVT = std::decay_t<_VT>;
        if (true) {
          return std::is_nothrow_invocable_r_v<_Res, _DVT _GLIBCXX_MOF_CV_REF, _ArgTypes...> &&
                 std::is_nothrow_invocable_r_v<_Res, _DVT _GLIBCXX_MOF_INV_QUALS, _ArgTypes...>;
        } else {
          return std::is_invocable_r_v<_Res, _DVT _GLIBCXX_MOF_CV_REF, _ArgTypes...> &&
                 std::is_invocable_r_v<_Res, _DVT _GLIBCXX_MOF_INV_QUALS, _ArgTypes...>;
        }
      }();
  ...
  public:
    ...
    : _Mofunc_base(static_cast<_Mofunc_base&&>(__x)),
      _M_invoke(std::exchange(__x._M_invoke, nullptr))
    { }

    /// Stores a target object initialized from the argument.
    template<typename _Fn, typename _Vt = std::decay_t<_Fn>>
      requires (!std::is_same_v<_Vt, move_only_function>)
      && (!std::__is_inplace_type<_Vt>::value) && __is_callable_from<_Vt>
      move_only_function(_Fn&& __f) noexcept(_S_nothrow_init<_Vt, _Fn>())
      {
        if constexpr (std::is_function_v<std::remove_pointer_t<_Vt>>
                      || std::is_member_pointer_v<_Vt>
    ...
    template<typename _Tp, typename... _Args>
      requires std::is_constructible_v<_Tp, _Args...>
      explicit move_only_function(std::in_place_type_t<_Tp>, _Args&&... __args)
    ...
    template<typename _Tp, typename _Up, typename... _Args>
      requires std::is_constructible_v<_Tp, std::initializer_list<_Up>&, _Args...>
      explicit
      move_only_function(std::in_place_type_t<_Tp>, std::initializer_list<_Up> __il,
                         _Args&&... __args)
      : _M_invoke(&_S_invoke<_Tp>)
      {
        static_assert(std::is_same_v<std::decay_t<_Tp>, _Tp>);
    ...
    template<typename _Fn>
      ...
      move_only_function&
      operator=(_Fn&& __f)
      noexcept(std::is_nothrow_constructible_v<move_only_function, _Fn>)
      {
    ...
    friend bool
    operator==(const move_only_function& __x, std::nullptr_t) noexcept
    { return __x._M_invoke == nullptr; }

  private:
    template<typename _Tp>
      using __param_t = std::conditional_t<std::is_scalar_v<_Tp>, _Tp, _Tp&&>;
